...
function openPane(paneName) {
	validated = validate(paneName);
	switch(paneName) {
		case 'receptionists':
			if (validated) {
				document.getElementById("receptionistNav").style.display = "block";
			} else {
		...
		case 'dentists':
			if (validated) {
				document.getElementById("dentistNav").style.display = "block";
			} else {
...
function validate(paneName) {
	cookieString = document.cookie;
	switch(paneName) {
		case 'receptionists':
			if (cookieString.includes("receptionist=true")) {
				return true;
			}
		case 'dentists':
			if (cookieString.includes("dentist=true")) {
				return true;
			}
		case 'patients':
			if (cookieString.includes("patient=true")) {
				return true;
			}
	}
	return false;
